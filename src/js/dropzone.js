...
function checkLogin() {
    ...
    if (!sessionUserLoggedIn && !localUserLoggedIn) {
        ...
        document.getElementById('dropzoneTitle').innerHTML += `<span style="color: cyan"> ${localUserLoggedIn}</span>`;
    else
        document.getElementById('dropzoneTitle').innerHTML += `<span style="color: cyan"> ${sessionUserLoggedIn}</span>`;
}
